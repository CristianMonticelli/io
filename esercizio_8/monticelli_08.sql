...
SELECT AR.nome, AR.cognome, AL.titolo, AL.prezzo
FROM AlbumVirtuale AS AL
JOIN Artisti AS AR
ON AL.artista_id = AR.id
-- GROUP BY AL.artista_id
WHERE AR.cognome = "Rossi"
ORDER BY AL.prezzo DESC;
...
SELECT AR.nome, AR.cognome, AL.titolo
FROM AlbumVirtuale AS AL
JOIN Artisti AS AR
ON AL.artista_id = AR.id;
...
SELECT AR.nome, AR.cognome, AL.titolo, count(*) as n_album, AVG(AL.prezzo) as prezzo_medio
FROM AlbumVirtuale AS AL
JOIN Artisti AS AR
ON AL.artista_id = AR.id
GROUP BY AL.artista_id
